...
- **Board tooling:** summary stat chips that double as **clickable filters** — Total (clears filters), Open (hides done/parked), quick-wins toggle, and per-status chips (click to filter the List to that status); a search box; dropdown filters for status (list view) / priority / effort / **tag**; sort by newest / priority / least-effort / quick-wins-first; and a collapsible **value-vs-effort matrix** (Quick wins · Big bets · Fill-ins · Time sinks). A **quick win** = high value (`impact`) + low effort (`complexity` S or M).
- **Detail page** big sections (Notes, Implementation scope, AI build prompt) are **collapsible** so a long idea doesn't force endless scrolling; the build prompt also scrolls within a capped-height box.
...
- **Generate build prompt:** SnapMeal's code is written entirely by an AI coding agent, never a human — so the detail page's `POST /api/ideas/[id]/plan` (route name kept; stored on `plan`/`plannedAt`) asks Claude (Opus 5) to produce a **self-contained build prompt addressed to the agent**, not a human handoff doc. The prompt is imperative ("Add… / Create… / Update…"), names the exact files/models/routes, bakes in the repo's conventions and deploy flow (branch → tsc + build → merge to main → push; additive schema only; `prisma generate`; owner-gating), and ends with acceptance criteria. The owner copies it and pastes it straight into Claude Code (or this chat) to build the feature — no edits needed.
